...
st.set_page_config(page_title='ANÁLISE MACRO', page_icon=':page_facing_up:',layout='wide') 


with st.sidebar:
    st.header('Análise Macro', divider='rainbow')
    st.image('360_F_650871606_h9it8XMfYK0Q2iaef5egbIfY263Ke4G8.JPG')
    st.divider()

    input_data_inicial = st.date_input('Selecione a Data Inicial de Pesquisa: ', format='DD/MM/YYYY', value=None)
    ...
    if media_retornos >= 0.01:
        st.metric(label="Retorno Acumulado no período:", value=f'{pontos_volatilidade} Pontos.', delta=f'{media_retornos:.2f}%')
